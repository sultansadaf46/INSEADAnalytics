...
1. 6
2. 0.93
3. The first 10 components reflect different types of financial assets and group different types of financial assets and markets
. 
Comp 1 : Europoean and North American Bonds 
Comp 2 : Safe Haven Currencies 
Comp 3: Large European Stock Exchanges 
Comp 4 : Smaller North American Stock Exchanges 
Comp 5 : European Financial Assets 
Comp 6: Oil & Gas 
Comp 7 : Regular Metal 
Comp 8 : Agricultural Commodities 
Comp 9 : Precious Metal 
Comp 10 : Asian Stock Exchanges 
...
The components are more highly correlated during crisis period and the scree plot is therefore steeper. This implies that fewer factors have eigenvalues greater than 1 during crisis. 
...
1. The code calcuoving averages of the future prices in different time windows. After that, it scales the price down so that there is $1/day investment across all futures. 

2. For MAfreq<-seq(15,250,by=20)

```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(15,250,by=20)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
