...
<br>
<br>
Lean Engine is an open-source fully managed algorithmic trading engine built for desktop and cloud usage. Link to github repository: https://github.com/QuantConnect/Lean
<hr>
<hr>
...
**Your Answers here:**
<br>
1. In the file dataSet1.R, line 8 has code mytickers = c("SPY", "AAPL") and the variable "mytickers" is later used to download Apple tickers as well. 
<br>
2. The cumulative returns of Apple during this period is `r round(100*sum(StockReturns[,2]),1)`%, the average daily return is `r round(100*mean(StockReturns[,2]),3)`%, and the standard deviation of the daily returns is `r round(100*sd(StockReturns[,2]),3)`%. 
<br>
3. The cumulative returns of Yahoo during this period is `r round(100*sum(StockReturns[,3]),1)`%, the average daily return is `r round(100*mean(StockReturns[,3]),3)`%, and the standard deviation of the daily returns is `r round(100*sd(StockReturns[,3]),3)`%. 
<br>
...
The number of columns in transposeData is 'r ncol(transposedData) and the number of rows is `r nrow(transposedData)`. 
...
1. The number of columns in transposeData is 'r ncol(transposedData) and the number of rows is `r nrow(transposedData)`. 
<br>
<br>
...
**Your Answers here:**
<br>
1. In dataSet1.R, we need to change the variable "startdate to 2001-01-01. 
<br>
<br>
<br>
...
**Your Answers here:**
<br>
1. It gives an error for incorrect number of dimensions. 
<br>
<br>
<br>
